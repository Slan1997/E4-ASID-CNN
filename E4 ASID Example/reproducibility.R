...
# setwd('~/E4-ASID-CNN/E4 ASID Example JHBI/')
folder_path='/Users/lanshi/Desktop/E4-ASID-CNN/E4 ASID Example JHBI'
setwd(folder_path)
library(pacman)
pacman::p_load(tibble,tidyr, dplyr, readr,lubridate,tfruns,zoo,abind,DescTools,
               rappdirs,scales,caret,magrittr,randomForest,mlbench,stringr)
...
source("functions.R") # load all the functions needed
...
dt_norm = read_csv(paste0(epoch_type[epo],"_permute_norm.csv")) %>%
  dplyr::select(all_of(var_lst)) %>% 
  mutate_at(c("sex","eda_cat","sleep"),as.factor) %>%
  mutate_at(c("hourofday","age"),scale)

time = read_csv(paste0(epoch_type[epo],"_permute.csv")) %>%
  dplyr::select(unix_sec)  # Get the exact time, will be used later for image plotting.

## handle subject 22's data issue
dt_norm1 = dt_norm %>% bind_cols(time) %>% 
  filter(!(e4_id==22& unix_sec>1582262785& unix_sec<1582316887))

dt_dum = dt_norm1 %>% dplyr::select(-unix_sec) %>% 
  dplyr::select(ncol(dt_norm),1:(ncol(dt_norm)-1)) 


################### Splits of training/validation/test sets
# The split of train/validation/test sets, denoted as “split,” follows 
# approximately the rule of thumb “80:20”: we randomly select 15 and 5 subjects 
# into train and validation sets (83%) respectively, and 4 subjects (17%) into
# test set. We repeat the random split process 5 times (denoted by S1, ..., S5), 
# and report mean and SD of performance metrics among splits for all the 
# classifiers built.

# here to show an example, only run one case with seed_split = 890
seed_split = 890
...
# Get the alignment of modalities and order of variables.
source("variable_order_rf.R")
